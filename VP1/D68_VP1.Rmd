...
---
We apply the fitness dynamic method of LeFrancq et al. to publicly available EV-D68 data.
...
D68_VP1_tree_dated <- read.nexus("nextstrain_enterovirus_d68_vp1_timetree.nexus")
## Make sure the tree is binary, and ladderized
D68_VP1_tree_dated <- collapse.singles(ladderize(multi2di(D68_VP1_tree_dated, random = F), right = F))

## Load metadata
D68_meta <- read.delim("nextstrain_enterovirus_d68_vp1_metadata.tsv", sep = "\t")
...
## Assign to tree
names_seqs <- D68_VP1_tree_dated$tip.label
D68_meta <- D68_meta[match(names_seqs, D68_meta$strain),]
D68_VP1_tree_dated$tip.label <- D68_meta$newname 

## Save tree
write.nexus(D68_VP1_tree_dated, file = "reformat_tree.nexus", translate = F)
...
```{r, eval=T}
genetic_distance_mat <- dist.nodes.with.names(D68_VP1_tree_dated)
```

Get the time of each internal node
```{r, eval=T}
nroot = length(D68_VP1_tree_dated$tip.label) + 1 ## Root number
...
colors_clade = c("#c4c4c4", "#3349f2", "#4a74f0", "#5ec6db","#33f27f", "#c6f233", "#fadb43", "#f29333", "#e8402a")
...
## Tree
#png('Index_fitness.png', 1400,1000, "px", res = 300)
par(mfrow = c(2,1), oma = c(0,0,0,0), mar = c(4,4,0,0))

min_year = 2000
max_year = 2023
plot(D68_VP1_tree_dated, show.tip.label = FALSE, 
...
start_time = Sys.time()
potential_splits = find.groups.by.index.dynamics(timed_tree = D68_VP1_tree_dated,
                                                 metadata = dataset_with_nodes,
                                                 node_support = D68_VP1_tree_dated$edge.length[match((n_seq+1):(2*n_seq-1), D68_VP1_tree_dated$edge[,2])],
                                                 threshold_node_support = 1/(906*0.004),
...
Optimize the number of groups: e.g. set the minimum number of sequences per group to 20, with a minimum frequency of 1%.
```{r, eval = T}
split = merge.groups(timed_tree = D68_VP1_tree_dated, metadata = dataset_with_nodes, 
                     initial_splits = potential_splits$potential_splits, 
                     group_count_threshold = 20, group_freq_threshold = 0.01)
```
